...
## Introduction
We want to merge the Full-Year Consolidated Data file with the Medical Conditions file so that we can identify patients with a diagnosis of HIV. 
...
knitr::include_graphics("Figure 3a.jpg")
```
...
knitr::include_graphics("Figure 3b.jpg")
```
...
For this exercise, we'll keep the `dupersid`, `varpsu.y`, `varsry.y`, `perwt20f.y`, and `icd10cdx` variables by using the `subset()` function. We'll call our reduced dataframe `keep_mep2`. 

```{r, echo = TRUE}
keep_meps2 <- subset(total, select = c("dupersid", "varpsu.y", "varstr.y", "perwt20f.y", "icd10cdx"))
```
...
We create the indicator and call it `diabetes`, which has is defined as `icd10cdx == "E11"`. We will code this as `0` for no diabetes and `1` for diabetes. Then, we count the number of time a patient as `E11` in their `icd10cdx` column. 
...
keep_meps2$diabetes[total$icd10cdx != "E11"] = 0
